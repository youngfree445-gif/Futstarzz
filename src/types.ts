...
export interface PlayerProfile {
   ...
   */
  ultimaEliminacion?: { competicion: string; semana: number };
  /**
   * Lo que publicaste vos en ChutSocial esta fecha. Una por fecha.
   * El feed lo muestra como tu post y responde segun el saldo (ver respuestasAMiPublicacion).
   */
  miPublicacion?: { texto: string; saldo: number; semana: number };
  /**
   * El club fichó a alguien de tu puesto y te toca pelear el lugar.
   *
   * `desdeSemana` marca cuando llego: el efecto se va aflojando solo a medida que pasan fechas, asi
   * que no hace falta un evento aparte para quitarlo. Ver refuerzoQueTeTapa en App.tsx.
   */
  fichajeRival?: { nombre: string; posicion: string; desdeSemana: number }; // semana en que respondiste la última conferencia de prensa -- una sola por semana, evita farmear prestigio infinito ciclando preguntas
  ...
  // El mercado de pases dejó de medir por reputación (1 a 5, donde Junior y el Real Madrid eran los
  // dos "5") y pasó a medir por la fuerza real del club. Una carrera creada antes de ese cambio se
