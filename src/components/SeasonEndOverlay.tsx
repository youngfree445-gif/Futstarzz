...
export default function SeasonEndOverlay({ info, onClose }: SeasonEndOverlayProps) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={`Fin de ${info.competition}`}
    >
      <div
      ...
      >
        ...
        <div className="w-20 h-20 mx-auto rounded-2xl bg-gradient-to-br from-slate-700 to-slate-800 flex items-center justify-center shadow-lg">
        ...
        <p className="mt-3 text-sm text-slate-300 leading-relaxed">
          <span className="font-bold text-white">{info.clubName}</span> cierra su participación en{' '}
          <span className="font-bold text-slate-200">{info.competition}</span>
          {!info.eliminated && info.finalPosition && info.totalTeams
