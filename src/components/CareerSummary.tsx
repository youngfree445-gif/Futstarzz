...
export default function CareerSummary({ playerProfile, onContinue }: CareerSummaryProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center py-12 px-4 relative">
      <div className="absolute top-1/4 left-1/4 w-[400px] h-[400px] bg-amber-500/5 rounded-full blur-[110px] pointer-events-none" />
        ...
        <div className="p-6 md:p-8 space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div className="bg-slate-950/60 p-4 rounded-2xl border border-slate-800 text-center">
              ...
              <span className="text-xl font-black text-emerald-400">{stats.golesHistoricos}</span>
            </div>
            <div className="bg-slate-950/60 p-4 rounded-2xl border border-slate-800 text-center">
              <span className="text-3xs text-slate-500 block font-mono uppercase">Asistencias</span>
              <span className="text-xl font-black text-amber-400">{stats.asistenciasHistoricos}</span>
            </div>
            <div className="bg-slate-950/60 p-4 rounded-2xl border border-slate-800 text-center">
          ...
          <div className="space-y-2">
            <h3 className="text-2xs uppercase tracking-widest text-slate-400 font-black flex items-center gap-1.5 border-b border-slate-800 pb-2">
              <Star size={14} className="text-amber-400" /> Trayectoria
            </h3>
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {playerProfile.seasonHistory.map((s, i) => (
                      <tr key={i} className={`border-t border-slate-800 ${s.titulo ? 'bg-amber-500/5' : ''}`}>
                        ...
                        <td className="px-3 py-2 text-center font-mono text-emerald-400">{s.goles}</td>
                        <td className="px-3 py-2 text-center font-mono text-amber-400">{s.asistencias}</td>
            ...
            {titulos > 0 && (
              <p className="text-3xs text-amber-400 font-mono uppercase text-center pt-1">
          ...
          <button
            onClick={onContinue}
            className="btn-fx w-full mt-2 py-4 px-5 rounded-2xl bg-gradient-to-r from-emerald-500 to-emerald-700 text-slate-950 font-black text-xs flex items-center justify-center gap-2 uppercase tracking-widest shadow-xl"
          >
